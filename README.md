...
| 태그 | 수정사항 |
|------|----------|
| **v0.1** | 배송/물류 관리 시스템 기본 - 거래처 CRUD, Excel import/export, 품목 관리, 플랜/루트/스탑 |
| **v0.2** | 품목 필드(product, weight, unit_price 등), 앱 설정(회사정보, 은행계좌), Geocoding(Kakao), 사용자 상태 |
| **v0.3** | 거래명세표, 미수금액, 은행설정, 스탑 지도 순서 핀 |
| **v0.4** | 스탑 지도 경로선, README 업데이트 |
| **v0.45** | 거래처 위도/경도 필드, 거래명세표(스탑/출발/사용자 정보 필드 추가) |
| **v0.5** | v0.45와 동일 |
| **v0.55** | 루트별 배송상태 표시: 플랜 목록에 1호차/2호차별 배송전·배송중·배송완료, 플랜 상세에서 각 루트 카드에 배송상태 표시 |
| **v0.6** | 선호언어(preferred_locale), 회원가입 시 언어 선택, 기사 탭(배정된 플랜, 내정보), admin 삭제 버튼 제한 |
| **v0.65** | route started_at, config API (/api/config/client), 배송 시작 API, 기사 앱: 다음 목적지/배달 목록 카드·스탑 접기, admin: 배송시작 상태 표시·기사 배정 모달·지도 크게 보기 모달·차량 핀, 배송 완료 시 미수금 반영 |
| **v0.66** | 기사 앱 UI 개선, admin 기사 배정 모달, 스탑 지도 클릭 시 크게 보기, 배송 완료 시 미수금 반영 |
| **v0.67** | 기사 앱: 다음 목적지 전체화면 패널, Leaflet 지도 임베드 |
| **v0.68** | 플랜 생성 시 기사 자동 배정(이전 플랜→사용자 탭 순서), 배달 목록 배송중 상태·순서 1부터, 배달 품목 하일라이트·"1박스" 형식 |
| **v0.7** | 기사 앱: 다음 목적지 Leaflet 지도, 카카오네비 링크, 배달 품목 강조 표시 및 단위(박스) 형식 통일 |
